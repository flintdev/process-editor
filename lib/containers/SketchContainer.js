import React from 'react';
import { DraggableContainer, DraggableChild } from 'react-dragline';
import LeaderLine from 'leader-line';
const lines = [];
export default class SketchContainer extends React.Component {
    componentDidMount() {
        setTimeout(() => {
            const a = document.getElementById('start');
            const b = document.getElementById('end');
            if (a && b) {
                lines.push(new LeaderLine(a, b));
            }
        }, 1);
    }
    render() {
        const children = [
            { id: 'start', position: { x: 100, y: 10 } },
            { id: 'end', position: { x: 400, y: 200 } },
        ];
        return (React.createElement(DraggableContainer, { style: { height: '100%', position: 'relative' } }, children.map(({ id, position }) => {
            const style = {
                width: 100,
                height: 100,
                cursor: 'move',
                background: '#8ce8df',
            };
            return (React.createElement(DraggableChild, { key: id, defaultPosition: position, onDrag: () => lines.forEach(line => line.position()) },
                React.createElement("div", { style: style, id: id })));
        })));
    }
}
